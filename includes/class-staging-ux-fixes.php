...
final class BubbaHubStagingUXFixes {
    ...
    public static function repair_managed_pages() {
        $map = [
            'home' => 'home', 'whats_on' => 'directory', 'buddy' => 'buddy',
            'myhub' => 'myhub', 'support' => 'support', 'about' => 'about',
            'leader' => 'leader', 'events' => 'events', 'compare' => 'compare',
            'antenatal' => 'antenatal', 'pricing' => 'pricing', 'signup' => 'signup',
        ];
    ...
    public static function remove_legacy_demo_records() {
        ...
        foreach ($demo as $post_type => $titles) {
            foreach ($titles as $title) {
                $ids = get_posts(['post_type' => $post_type, 'post_status' => 'any', 'title' => $title, 'posts_per_page' => -1, 'fields' => 'ids', 'no_found_rows' => true]);
    ...
    public static function enqueue_polish() {
        ...
        if (!$is_app) return;
        wp_enqueue_style('bubbahub-figma-polish', BUBBAHUB_URL . 'assests/css/figma-polish.css', ['bubbahub'], BUBBAHUB_VERSION);
        wp_enqueue_script('bubbahub-figma-polish', BUBBAHUB_URL . 'assests/js/figma-polish.js', ['bubbahub'], BUBBAHUB_VERSION, true);
        wp_enqueue_script('bubbahub-figma-details', BUBBAHUB_URL . 'assests/js/figma-details.js', ['bubbahub-figma-polish'], BUBBAHUB_VERSION, true);
    }
}
