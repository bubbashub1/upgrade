...
require_once BUBBAHUB_DIR.'includes/class-leader-group-manager.php';
require_once BUBBAHUB_DIR.'includes/class-directory-views.php';
...
add_action('plugins_loaded', function(){
  ...
  if(version_compare($stored_version,BUBBAHUB_VERSION,'<')){
    try{
      BubbaHubSafeInstaller::install();
    }catch(Throwable $e){
      error_log('BubbaHub safe upgrade failed: '.$e->getMessage());
    }
  }
  new BubbaHub();
},1);
